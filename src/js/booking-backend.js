...
(function() {
    ...
    async function checkAuth() {
        try {
            ...
            if (!response.ok) {
                console.error('Error al verificar autenticación:', response.status, response.statusText);
    ...
    function initBooking() {
        ...
        try {
            ...
            if (bookingForm) {
                ...
                console.log('✅ Event listener de submit registrado en el formulario');
                ...
                if (submitBtn) {
                    ...
                    submitBtn.addEventListener('click', (e) => {
                        console.log('🔵 Click en botón de submit detectado');
                        console.log('🔵 Estado isSubmitting:', isSubmitting);
                        
                        // Si ya hay una reserva en proceso, prevenir
                        if (isSubmitting) {
                            console.warn('⚠️ Ya hay una reserva en proceso, bloqueando click...');
                            e.preventDefault();
                            e.stopPropagation();
                        ...
                        if (!bookingForm.checkValidity()) {
                            console.warn('⚠️ Formulario no válido, dejando que HTML5 muestre los errores');
                        ...
                        setTimeout(() => {
                            if (!isSubmitting) {
                                console.log('⚠️ El submit no se disparó automáticamente, disparándolo manualmente...');
                                const submitEvent = new Event('submit', { bubbles: true, cancelable: true });
                                bookingForm.dispatchEvent(submitEvent);
                            }
                        }, 100);
                        
                        console.log('✅ Click permitido, el submit del formulario se disparará');
                    }, { capture: false }); // No usar capture para no interferir
                } else {
                    console.warn('⚠️ No se encontró el botón de submit en el formulario');
                }
            } else {
                console.error('❌ No se encontró el formulario #bookingForm');
            }
    ...
    function showSuccessModal(details) {
        console.log('🎉 Mostrando modal de éxito con detalles:', details);
        
        const successModal = utils.getElement('#successModal');
        const successDetails = utils.getElement('#successDetails');
        
        if (!successModal) {
            console.error('❌ No se encontró el modal de éxito (#successModal)');
        ...
        if (!successDetails) {
            console.error('❌ No se encontró el contenedor de detalles (#successDetails)');
        } else {
            // Llenar detalles
            ...
            `;
            console.log('✅ Detalles del modal actualizados');
        }
        ...
        console.log('✅ Modal de éxito mostrado, aria-hidden:', successModal.getAttribute('aria-hidden'));
        ...
        if (closeBtn) {
            ...
            console.log('✅ Listener de cerrar configurado');
        } else {
            console.warn('⚠️ No se encontró el botón de cerrar (#successModalClose)');
        }
        
        if (okBtn) {
            okBtn.onclick = closeSuccessModal;
            console.log('✅ Listener de OK configurado');
        } else {
            console.warn('⚠️ No se encontró el botón OK (#successModalOk)');
        }
        
        if (overlay) {
            overlay.onclick = closeSuccessModal;
            console.log('✅ Listener de overlay configurado');
        } else {
            console.warn('⚠️ No se encontró el overlay del modal');
        }
        ...
        setTimeout(() => {
            if (okBtn) {
                okBtn.focus();
                console.log('✅ Foco dado al botón OK');
            }
        }, 100);
    }
    ...
    async function handleBookingSubmit(e) {
        console.log('🔵 ========== handleBookingSubmit INICIADO ==========');
        ...
        console.log('🔵 handleBookingSubmit llamado después de preventDefault');
        
        // Prevenir doble envío
        if (isSubmitting) {
            console.warn('⚠️ Ya hay una reserva en proceso, ignorando...');
            return false;
        }
        ...
        try {
            ...
            if (result && result.success) {
                console.log('✅ Reserva exitosa, preparando modal de éxito...');
                ...
                console.log('📋 Detalles de la reserva exitosa:', successDetails);
                ...
                console.error('❌ El backend no reportó éxito:', errorMsg);
            ...
            if (error.message && error.message.includes('Error desconocido')) {
                console.warn('⚠️ El backend puede haber procesado la reserva pero no devolvió success=true');
                ...
                try {
                    ...
                    console.error('❌ Error al mostrar modal de éxito:', modalError);
                }
            }
